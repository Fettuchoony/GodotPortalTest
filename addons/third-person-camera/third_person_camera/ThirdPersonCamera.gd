...
##
@export_range(1.0, 1000.0) var vertical_rotation_sensitiveness := 10.0
...
var camera_vertical_rotation_deg := 0.
...
func _ready():
	...
	# Lock mouse in place, can esape in player movement settings
...
# TODO: edit to vertical instead of horizontal
# DONE: Updated successfully i think?
func _process_vertical_rotation_input() :
	if follow_target == FOLLOW_TARGETS.PARENT :
		return
	if InputMap.has_action("tp_camera_up") and InputMap.has_action("tp_camera_down") :
		var camera_vertical_rotation_variation = Input.get_action_strength("tp_camera_up") -  Input.get_action_strength("tp_camera_down")
		camera_vertical_rotation_variation = camera_vertical_rotation_variation * get_process_delta_time() * 30 * vertical_rotation_sensitiveness
		camera_vertical_rotation_deg += camera_vertical_rotation_variation

# TODO: fix for vertical rotation to track properly
...
# Make camera more ror2 like, created by Fettuchoony 
func _update_camera_vertical_rotation() :
	if follow_target == FOLLOW_TARGETS.PARENT :
		return
	var tween = create_tween()
	tween.tween_property(_camera_rotation_pivot, "global_rotation_degrees:x", camera_vertical_rotation_deg * -1, 0.1).as_relative()
	camera_vertical_rotation_deg = 0.0 # reset the value
	var vect_to_offset_pivot : Vector2 = (
		Vector2(_camera_offset_pivot.global_position.y, _camera_offset_pivot.global_position.z)
		-
		Vector2(_camera.global_position.y, _camera.global_position.z)
...
func _unhandled_input(event):
	if follow_target == FOLLOW_TARGETS.MOUSE and event is InputEventMouseMotion:
		...
		# Vertical component
		camera_vertical_rotation_deg += event.relative.y * 0.1 * mouse_y_sensitiveness
		#camera_tilt_deg -= event.relative.x * 0.07 * mouse_y_sensitiveness
		return
